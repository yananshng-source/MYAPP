    ...
    for group_key, group_data in df_source.groupby(grouping_columns):
        ...
        if pd.notna(最低分) and '最低分' in group_data.columns:
            ...
            if not min_score_rows.empty:
                ...
                院校招生代码 = min_score_row.get('院校招生代码', '') if '院校招生代码' in min_score_row else ''
        ...
        if not 数据来源 and len(group_data) > 0:
            ...
            院校招生代码 = first_row.get('院校招生代码', '') if '院校招生代码' in first_row else ''
        ...
        result_row = {
            ...
            '招生人数': 招生人数,
            '数据来源': 数据来源,
            '专业组代码': 专业组代码,  # 这个在分组中已经处理过
            '首选科目': 首选科目,
            '院校招生代码': 院校招生代码
        }

        # 检查是否有其他需要处理的列（除了已经处理的列之外）
        processed_columns = set(result_row.keys())
        ...
        # 如果有其他列在源数据中但不在我们的结果中，也添加到结果
        for col in group_data.columns:
            if col not in processed_columns and col not in grouping_columns:
                # 对于新列，使用组内第一条记录的值
                if len(group_data) > 0:
                    result_row[col] = group_data.iloc[0].get(col, pd.NA)
                else:
...
with tab6:
    ...
    - **固定输出列（一定有值）**：
      - 学校名称、省份、招生类别、层次、招生批次

    - **根据源数据决定的列（有值就处理，没值就置空）**：
      - 招生类型、最高分、最低分、最低分位次、录取人数、招生人数、数据来源、专业组代码、首选科目、院校招生代码
      - 以及其他源数据中存在的列
    ...
    uploaded_file_admission = st.file_uploader(
        "上传招生数据Excel文件",
        type=['xlsx'],
        help="请上传包含招生数据的Excel文件，系统会自动识别和处理所有列",
        key="admission_excel"
    )
    ...
    if uploaded_file_admission is not None:
        try:
            ...
            if st.button("🚀 开始处理招生数据", type="primary", key="admission_btn"):
                ...
                # 显示所有输出列
                st.write(f"**输出列名 ({len(result_df.columns)}列):** {list(result_df.columns)}")
